...
```{r}
# conver to data frame, just cause
...
dplyr_sel <- select(mpg, manufacturer, model, cty, hwy)
head(dplyr_sel)
...
dplyr_four_cyl <- filter(mpg, cyl == 6)
head(dplyr_four_cyl)
```
...
Now that you know the basic functions in `dplyr` and how to use them, let's apply them to a more realistic dataset.  We're going to import a sediment chemistry dataset from Bight13.  For this exercise, let's pretend we need to get a subset of the data that we're interested in. We'll be getting all of the observations for chlorophyll (`Total_CHL`) and getting rid of everything else. 
...
2. Get a feel for the data.  What are the dimensions (`dim`)? What are the column names (`names`)? What are the types of data in each column (`str`)?

3. Now we need to think about what variables we need to get the chlorophyll data.  Select the `StationID`, `Parameter`, and `Result` columns and save it as a new object.

4. Let's filter the observations (i.e., `Parameter`) to get only chorophyll (`Total_CHL`). Use the object you saved from the last step to filter.  Again, save the new object when you've successfully filtered the data.

5. Now that we've filtered, we don't need the Parameter column.  Select only the `StationID` and `Result` columns, save it as a new object. 

6. Finally, let's change the names of the columns to something simple and more informative. Rename `Result` to `chl`.

7. When you're happy with the result, have a look at the data.  Did you get what you wanted?  How many observations do you have compared to the original data file? Save the new file to your data folder using `write_csv`.
