...
1. Make a folder in your project called `data` if don't have one already. Place the formatted data ([download](https://github.com/SCCWRP/R_training_2018/raw/master/lessons/data/formatted_data.xlsx)) into this folder.
...
Let's make some simple graphics of our chemistry dataset using base R.  We'll make a scatterplot of chlorophyll versus total organic carbon using the `plot` function.  We'll also add a title and better axis labels to the plots to make them slightly more palatable.  
...
With ggplot2, you begin a plot with the function `ggplot()`. `ggplot()` creates a coordinate system that you can add layers to. The first argument of `ggplot()` is the dataset to use in the graph. So `ggplot(data = mpg)` creates an base graph.
...
More commonly, the `aes` function that defines the mapping is included in the initial call to `ggplot`.  This will globally define the mapping to all geoms in a plot, instead of for only one geom. There are reasons to globally apply the aesthetics or separately for each geom.   
...
In all of the above examples we've mapped an aesthetic to a variable in our dataset. We can just as easily modify the plot without mapping it to a variable (i.e., just change a part of the plot independent of the data).  For example, maybe we want to change the color of the points using a single color.  Notice the placement of `colour` outside of the `aes` mapping function.
```{r}
ggplot(data = mpg, aes(x = displ, y = hwy)) + 
...
There are countless ways we can modify a ggplot, either by manipulating the appearance or adding information that improves our understanding of what we see.  In the previous exercise, we used a scale to transform an axis.  In this next section we'll cover two additional ways to modify a plot:
...
First I'll show you how to modify the appearance to your liking. Individual components can be modified with `theme`.  Here the legend is moved to the top, the minor grid lines between axis ticks are removed,and the panel background is changed from gray to white. Check the help file for `theme` to see all the options.
