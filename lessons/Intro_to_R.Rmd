...
We only have two hours to introduce basic concepts in R. I's an unrealistic expectation that you will be competent after only two hours.  As such, the goals of this training are to expose you to fundementals and to develop an appreciation of what's possible with this software.  I also want to provide resources that you can use for follow-up learning on your own. 
...
* The growth of R as explained in the [Stack Overflow blog](https://stackoverflow.blog/2017/10/10/impressive-growth-r/)
...
In the old days, the only way to use R was directly from the Console - this is a bare bones way of running R only with direct input of commands. Now, RStudio is the go-to Interactive Development Environment (IDE) for R. Think of it like a car that is built around an engine. It is integrated with the console and includes many other features such as version control, debugging, dynamic documents, package manager and creation, and code highlighting adn completion. 
...
In most cases, you will not enter and execute code directly in the console.  Code can be created in a script and then sent directly to the console when you're ready to run it.  The key difference here is that a script can saved and shared.

Open a new script from teh File menu...
...
6. Lastly, we need to get this project set up with some example data for our upcoming exercises.  You should have downloaded this already, but if not, the data are available from <https://sccwrp.github.io/R_training_2018/data.zip>.  Copy this zip file into your project and unzip into a folder named `data`.  

## R language fundamentals

The basic syntax of function follows the form: `function_name(arg1, arg2, ...)`.  
...
While technically this is the same as `<-`, it's generally considered poor practice in code writing.  The `=` sign in R is typically eserved for arguments within functions:
...
- [R-sig-ecology](https://stat.ethz.ch/mailman/listinfo/r-sig-ecology): A special interest group for use of R in ecology.  Less daunting the the main help with participation from some big names in ecological modelling and statistics (e.g., Ben Bolker, Gavin Simpson, and Phil Dixon).
...
As I mentioned earlier, there are TOO many resources to listy here and everyone has their favorites.  Below are just a few that I like.
...
Now that you know how to get started in R and where to find resources, we can begin talking about R data structures. Simply put, a data strucure is a way for programming languages to handle storing information.
...
The four types of atomic vectors (think 'atoms' make up a molecule) are `double` (or numeric), `integer`, `logical`, and `character`.  For most purposes you can ignore the `integer` class, so there are basically three types.  Each type has some useful properties:
...
A more common way of storing two-dimensional data is in a data frame (i.e., `data.frame`). As you probably guessed, these are similar to a matrix with the added bonus of combining data of different types.  Think of them like your standard spreadsheet, where each column describes a variable.  Here's a simple example:
...
As you might have guessed, a requirement of a matrix or data frame is that all columns have the same number of rows.  Vectors can combined as a `list` if they have unequal length. In fact, a data frame is simply a special case of a list where all the vectors have the same length (try `as.list(mydf)`).  
...
Lists have the least constraints on the types of data that can be stored and therefore are the most flexible of the two-dimensional structures.  In fact, you can create an n-dimensional list by creating lists of lists, lists of list of lists, etc.  You will probably not use them in your early R adventures, but they are a very powerful option for data storage. 

## Data I/O

It is the rare case when you manually enter your data in R.  Most data analysis workflows typically begin with importing a dataset into R.  Literally, this means committing a dataset to memory in R as one of it's accepted data structure format.

If you're lucky, your data are already in a flat format.  In other words, your data are pure and are stored only using standard ASCI text.  Flat data files present the least complications on import because there is very little to assume about the structure of the data. On import, R tries to guess the data type for each column and this is fairly unambiguous with flat files.  

More often you will probably be given an Excel spreadsheet to import.  In the old days, importing spreadsheets into R was almost impossible given the proprietary data structure of Excel.  The tools available in R have since matured and it's now pretty painless to export a spreadsheets into R.  But "painless" comes with some caveats.  
...
Computers, although very trainable, are also very dumb.  If you have trouble interpreting the information, can you expect a computer to do so?  Here are few pointers to help the data import from Excel go smoothly:

* Use the one spreadsheet, one data frame rule
...
Assuming the data are in a format that can be easily read, we can import it using the `readr` package. 

## About retention...

## Attribution

Content in this lesson was pillaged extensively from Jeffrey Hollister and the USGS-R training curriculum [here](https://github.com/USGS-R/training-curriculum).