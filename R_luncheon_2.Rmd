...
Okay, so there's one station in our chemistry data that wasn't in our location data.  That's okay and an acceptable loss.  We'll use an `inner_join` to combine the two datasets by the station id.  This will drop all stations that aren't shared between the two datasets.   
...
We'll create a pairs plot by evaluating everything except the station column.  We'll set the argument `gap = 0` to minimize gaps between the plot panels to help us better see the plots. The argument `cex = 0.5` reduces the point size. Pro tip: you might get an error <span style="color:red">Error in plot.new() : figure margins too large</span>.  This simply means that the figure margins in RStudios plot window (bottom right panel) are too small to plot the data.  Use the `windows()` function from the command line to make a new graphics device that you can expand. 
...
We'll want to remove these by hand since these will likely cause problems, i.e., skewing correlation coefficients, linear models, etc. This is most easily done by selecting the variables from the dataset, subsetting by the offending values, and assigning an `NA` value.
...
Now we can start addressing our research questions. Our first question is whether or not any of thes variables following some kind of spatial pattern.  This would suggest somethig about location is affecting sediment toxicity, such as different sources of pollutatns or different management patterns.  We can get a visual representation of these patterns by looking at how they vary by latitude and longitude. 
...
Now let's overlay some of the survivability estimates on the points.
```{r}
...
We can use the `ggpairs` function to make a beefed up version of the pairs plot.  In addition to paired scatterplots, you can get a better idea of the distribution of each variable and actual estimates of correlations between pairs of variables. We'll not look at latitude or longitude for this example. 
