...
The next step is to join the sediment chemistry data with the station data.  We'll have to wrangle the chemistry data first to remove some extra columns and filter by a parameter of interest.  We'll filter the arsenic samples, but you could filter the chemistry data by any of the other parameters.
...
We also might want to add a scale bar and north arrow (as if you couldn't tell which way was up).  This has never been easy in R but there are some newer tools that make this "easier", although it's still requiers a bit of hacking to get the locations right.  The [ggsn](http://oswaldosantos.github.io/ggsn/) package has made this process a litte simpler.  You'll need to download the development version from GitHub.
...
This map now includes all standard cartographic objects that are appropriate for a publication ready map. We also have some idea about where arsenic concentrations are highest and if the marine protected areas correspond to these areas.  
...
To use `ggmap`, you have to first download the image with the `get_map` function based on the extent of the plotting area.  This requires pulling the extent (max/min coordinates) from our existing data object.  For this example we'll download a satellite image of our area.  One thing to keep in mind is the `zoom` argument for `get_map`.  This is used to specify the level of detail in your downloaded image and can take some time to download depending on the area and level of specificity.  We'll follow these steps to download the basemap:
...
We can make a few minor tweaks to the map for the final format for readability, including some color changes: polygon outlines as white, scale bar with white text, different north arrow, and shortening the y-axis limits.
...
The above approach works well for creating static maps as stand-alone products. However, there are a few drawbacks that limit it's functionality.  First, the amount of coding to the get the final product is a bit excessive.  The final product was created with several packages and the default aesthetics required a lot of tweaking to make them presentable.  Second and more importantly, this map is not interactive and only provides a snapshot of conditions.  We can address both of these issues with the [mapview](https://r-spatial.github.io/mapview/) package.
...
With some minor changes, we can recreate our static map above with mapview. We can also change the labels that popup on mouseover for each point and polygon.
...
# mapview
mapview(stations, zcol = 'Result', layer.name = 'Arsenic (ug/g)', lwd = 0, col.regions = cols, label = paste(stations$Station, stations$Year, stations$Result)) + 
  mapview(polys, layer.name = 'MPA', color = 'red', alpha.regions = 0, label = polys$MPA_Name)
```

There's a lot more we can do with mapview but the point is that these maps are incredibly easy to make with `sf` objects and they offer a lot more functionality.  Now that we've created these maps with RMarkown, we can also compile the document into a single file that we can share with anybody.  This can go a long way for making the data accessible to new audiences. 
