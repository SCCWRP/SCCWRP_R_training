...
To emphasize the point that the `sf` package plays nice with the tidyverse, let's do a simple feature on the stations data to get only teh stations that were sampled 2013.
...
Now let's use the stations and MPA polygons to conduct a quick geospatial analysis.  Our simple question is as follows:

__How many stations were sampled in each MPA and in which year?__

The first task is to subset the stations data (all years) by locations on the Bight that are MPAs.  There are a few ways we can do this.  The first is to make simple subset where we filter that stations data based on a spatial overlap.  The second and more complicated approach is to intersect the two data objects to combine the attributes of both.
...
With this first approach we can see which stations were located in an MPA, but we don't know the name of the MPA for each station.
